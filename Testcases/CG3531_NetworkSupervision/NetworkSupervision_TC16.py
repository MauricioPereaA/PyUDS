...
class PyUDS_TestCase(unittest.TestCase):
    ...
    def test_012(self, name='Read DTCs'):
        test.preconditions(
            step_info=info()
        )
        
        #Step 9 Power Supply - Turn OFF
        ...
        #Step 10 Power Supply - Turn ON
        test.power_supply.output(True)

        #Step 11 Send NMF - Restablish communication        
        #test.restart_communication(only_nmf=True)
        
        #Transmit safety security protection Invalid:
        test.canoe.set_envVariable(Env_MsgID = self.Msg_ID)
        test.canoe.set_system_variable('SysVarSecMsg','SysInValidDataCount','1')
        test.canoe.set_system_variable('SysVarSecMsg','SysInvalidARC','1')
        test.canoe.set_envVariable(Env_MsgStartStop='INVALIDARC')
    ...
    def test_013(self, name='wait +4 seconds and read DTCs - InvalidData DTC is set'):
        test.preconditions(
            step_info=info()
        )
        time.sleep(5)
        ...
        prompt = tools.popup.ask(title=name, description='Please check No InvalidSignal DTC are set within 5 seconds from power up')
