...
class PyUDS_TestCase(unittest.TestCase):
    ...
    def test_001_1(self, name='Start Test'):
        ...
        for msg in self.invalid_signal_DTCs[device_under_test].keys():
            print(msg)
            if self.invalid_signal_DTCs[device_under_test][msg]['CS'] == 1:
                ...
                #Step 14 Start and continue requesting DTCs, at the recorded periodic rate, for the recorded received message that contains CS Signal
                ...
                #Step 15 Start and continue transmitting the recorded received message with an invalid CS value
                test.canoe.set_envVariable(Env_MsgID = self.Msg_ID)
                test.canoe.set_system_variable('SysVarSecMsg','SysInValidDataCount','1')
                test.canoe.set_system_variable('SysVarSecMsg','SysInvalidCS','1')
                test.canoe.set_envVariable(Env_MsgStartStop='INVALIDCS')
                time.sleep(6)
                ...
                #Step 16 Once the InvalidSignalDataReceived DTC sets, start transmitting the supervised message with a valid CS value 
                test.canoe.set_envVariable(Env_MsgID = self.Msg_ID)
                test.canoe.set_system_variable('SysVarSecMsg','SysInValidDataCount','0')
                test.canoe.set_system_variable('SysVarSecMsg','SysInvalidCS','0')
                test.canoe.set_envVariable(Env_MsgStartStop='INVALIDCS')
                time.sleep(3)
                
                #Step 17 Stop requesting DTCs
                test.canoe.set_system_variable('Generic_Functional','TriggerDTCRead','0')
                
                self.count_step()
                #Manually check in RBS trace if the DTC testFailed bit goes to 0 after two instances of Valid CS
                prompt = tools.popup.ask(title=name, description='Verify invalidSignal DTCs status byte is 2E after two instances of Valid CS')
                test.compare(True, prompt, step=self.current_step()+' - Verify invalidSignal DTCs status byte is 2E after two instances of Valid CS')      
                ...
                #Step 21 Transmit the received message with the following Invalid/Valid CS pattern:
                #less than X_Failure_Count Limit number of messages with invalid CS (Invalid):
                ...
                #followed by 2 times X_Failure_Count Limit number of messages with Valid CS (Valid) :               
                ...
                if invalid_msgs_count >0:
                    ...
                    test.step(
                        step_title='{0} - {1}'.format('Read DTC - Must be clear after 10 cycles of "Invalid = less than x" and "Valid = x*2" CS Pattern', device_under_test)+'::'+msg,
                        custom='19 02 09',
                        expected={
                            'response': 'Positive',
                            'data': 'FF'
