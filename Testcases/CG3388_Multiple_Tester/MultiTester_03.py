...
class PyUDS_TestCase(unittest.TestCase):
    ...
    def test_004(self, name='Multiple Tester 3'):

        # *CHECKED* - Check-box
        test.canoe.set_envVariable(MultiTester3=1)

        # *PUSH* - Run selected test cases
        test.canoe.set_envVariable(MultiTester=1)
        time.sleep(0.75)
        test.canoe.set_envVariable(MultiTester=0)
        test.canoe.set_envVariable(MultiTester3=0)

        # Wait for Transport Layer Test Case to be finished
        print('Please wait until TC is completed...')
        time.sleep(5)
        test.canoe.stop()
        test.compare(self.MultiTester.test_case_003(), True, 
                        step='test_004')
    def test_005(self, name='Multiple Tester 3'):
        test.compare(self.MultiTester.test_case_003(), True, 
                        step='test_005') 
    def test_006(self, name='Multiple Tester 3'):
        test.compare(self.MultiTester.test_case_003(), True, 
                        step='test_006') 
    def test_007(self, name='Multiple Tester 3'):
        test.compare(self.MultiTester.test_case_003(), True, 
                        step='test_007') 