...
class PyUDS_TestCase(unittest.TestCase):
    ...
    def test_016(self, name='Multiple Tester 7'):

        # *CHECKED* - Check-box
        test.canoe.set_envVariable(MultiTester7=1)

        # *PUSH* - Run selected test cases
        test.canoe.set_envVariable(MultiTester=1)
        time.sleep(0.75)
        test.canoe.set_envVariable(MultiTester=0)
        test.canoe.set_envVariable(MultiTester7=0)

        # Wait for Transport Layer Test Case to be finished
        print('Please wait until TC is completed...')
        time.sleep(5)
        test.canoe.stop()
        #print(self.MultiTester.test_case_007())
        test.compare(self.MultiTester.test_case_006(), True, 
                        step='test_016')
    def test_017(self, name='Multiple Tester 7'):
        test.compare(self.MultiTester.test_case_006(), True, 
                        step='test_017') 
    def test_018(self, name='Multiple Tester 7'):
        test.compare(self.MultiTester.test_case_006(), True, 
                        step='test_018') 