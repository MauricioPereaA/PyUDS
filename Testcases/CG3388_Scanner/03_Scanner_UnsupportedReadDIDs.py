...
class PyUDS_TestCase(unittest.TestCase):
    ...
    def test_001(self, name='read all dids'):
        test.preconditions(step_info=info())
        #for i in range(65536):
        for i in range(6):
            supported = True if self.format_hex(i) in self.supported_dids else False
            if not supported:
                test.step(
                    step_title='Read ' + self.format_hex(i),
                    custom='22 ' + self.format_hex(i),
                    expected={
                        'response': 'Negative',
                        'data': '31'
                    }
    ...
    def test_002(self, name='read dids which deviate from GB6000'):
        if not len(self.read_dids_deviate_from_ODX) > \
            self.read_dids_deviate_from_ODX.count('THIS IS A PLACEHOLDER'):
            print('Theres no read dids that deviates from GB6000\n',
                    'if any, please specify from %s test script.'%__name__)
            return 0
        for read_dids in self.read_dids_deviate_from_ODX:
                test.preconditions(
                    step_info=info()
                )
                test.step(
                    step_title=read_dids,
                    custom='22 '+ read_dids,
                    expected={
                        'response': 'Negative',
