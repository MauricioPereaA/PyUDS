...
class Gen_Boot_Requirements(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        message = 'Please ensure you have APP SW Flashed on the ECU.'+\
                  '\nThis test is intented to Err LengthExceeded when programming APP SW that exceeds expected length'
        print(__name__, message)
        tools.popup.warning(title='Bootloader',
                                description=message)
        
        self.limit_iterations = 371 # Tested on the __Examples__ // test23 with the file that exceeds the expected binary size
                           # This number comes from that test
    @classmethod
    ...
    def test_007(self, name='Transfer Data modified App SW file (Bootloader receives more data than allowed)'):
